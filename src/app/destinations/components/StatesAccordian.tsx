...
export function StatesAccordion({ states }: StatesAccordionProps) {
  ...
  const handleCityClick = (stateSlug: string, citySlug: string) => {
      router.push(`/destinations/${stateSlug}/${citySlug}`);
  };
    ...
    <div className="bg-white rounded-3xl p-8 shadow-xl border border-gray-100">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mb-6">
          Select a state to explore cities and monuments
        </p>
        ...
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 transition-all"
          />
        </div>
      </div>
      ...
      <div className="space-y-3">
        {filteredStates.map((state) => {
          ...
          return (
            <div
              key={state.slug}
              className="border-2 border-gray-100 rounded-2xl overflow-hidden hover:border-orange-200 transition-all"
            >
              {/* State Header */}
              <button
                onClick={(e) => handleStateHeaderClick(e, state.slug)}
                className="w-full flex items-center justify-between p-5 bg-gradient-to-r from-gray-50 to-white hover:from-orange-50 hover:to-orange-50/50 transition-all"
              >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {filteredCities.map((city) => (
                        <button
                          ...
                          className="group flex items-center gap-3 p-4 bg-gray-50 hover:bg-orange-50 rounded-xl transition-all hover:shadow-md border border-gray-200 hover:border-orange-300"
                        >
                          <div className="bg-white p-2 rounded-lg group-hover:bg-orange-100 transition-colors">
                            <Building2 className="w-4 h-4 text-gray-600 group-hover:text-orange-600" />
                          </div>
