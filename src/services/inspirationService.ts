...
export const getInspirationalTours = async (): Promise<ApiResponse<ThemeGroup[]>> => {
  try {
    ...
    let themes = themesResponse.success && Array.isArray(themesResponse.data) ? themesResponse.data : [];
    ...
    for (const theme of themes) {
      try {
        ...
        const toursResponse = await handleApiCall<ApiResponse<{ tours: Array<{ id: string; title: string; description: string }> }>>(
          async () => axios.get(`${endpoints.tours.getTours}?themeId=${theme.id}&limit=3`),
          `Tours for ${theme.name} fetched successfully`
        );

        if (toursResponse.success && toursResponse.data && Array.isArray((toursResponse.data as unknown as { tours: Array<{ id: string; title: string; description: string }> }).tours)) {
          const tours = (toursResponse.data as unknown as { tours: Array<{ id: string; title: string; description: string }> }).tours;
          themeGroups.push({
            id: theme.id,
            name: theme.name,
    ...
    case 'culture':
      return [
        ...
        },
        {
          ...
          description: "Follow the footsteps of Buddha through significant Buddhist sites in India.",
          url: "/",
          themeId: "culture",
          theme: "Cultural Tours"
        }
